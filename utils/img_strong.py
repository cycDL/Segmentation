...
def get_2dshape(shape, *, zero=True):
    if not isinstance(shape, collections.Iterable):  # collections.Iterable是list,tuple,dict,char，string等
        shape = int(shape)
        shape = (shape, shape)
    else:
        h, w = map(int, shape)
...
def random_crop_pad_to_shape(img, crop_pos, crop_size, pad_label_value):
    ...
    img_crop = np.array(img[start_crop_h:start_crop_h + crop_h, start_crop_w:start_crop_w + crop_w, ...], np.float)
...
def generate_random_crop_pos(ori_size, crop_size):  # ori_size:(1024,756) , crop_size(512,512)
...
# img：裁剪过后的图  shape:(512,512), border_mode:cv2.BORDER_CONSTANT(固定值边缘填充), value：0或255 想要填充的数 可以是(255,255,255)三通道的
def pad_image_to_shape(img, shape, border_mode, value):
    ...
    src:源图像
    top,bottem,left,right: 分别表示四个方向上边界的填充长度
    borderType: 边界的类型
    有以下几种：
...
def random_scale(img, gt, scales):
    scale = random.choice(scales)  # scales=[0.5, 0.75, 1.0, 1.5, 1.75, 2.0]从scales中随机选取一个
...
"""
cv2.flip(img,value)
img:读取的图片，img=cv2.imread(path)
value:= 0, for flipping the image around the x-axis (vertical flipping);
